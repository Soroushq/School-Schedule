...
import { ELEMENTARY_GRADES, ELEMENTARY_CLASS_OPTIONS, ELEMENTARY_FIELDS, ELEMENTARY_TEACHING_GROUPS } from '../constants/elementaryConstants';

// تایپ داده‌های کانتکست
interface ElementaryScheduleContextType {
  // داده‌ها
...
// ایجاد کانتکست
const ElementaryScheduleContext = createContext<ElementaryScheduleContextType | undefined>(undefined);

// پراپس‌های کامپوننت Provider
interface ElementaryScheduleProviderProps {
  children: React.ReactNode;
}

// کامپوننت Provider
export const ElementaryScheduleProvider: React.FC<ElementaryScheduleProviderProps> = ({ children }) => {
  // استیت‌های داده
  ...
  // مقدار کانتکست
  const contextValue: ElementaryScheduleContextType = {
    // داده‌ها
    personnelList,
    personnelSchedules,
    classSchedules,
    ...
    classOptions: ELEMENTARY_CLASS_OPTIONS,
    fields: ELEMENTARY_FIELDS,
    teachingGroups: ELEMENTARY_TEACHING_GROUPS,
  ...
  return (
    <ElementaryScheduleContext.Provider value={contextValue}>
      {children}
    </ElementaryScheduleContext.Provider>
  );
};

// هوک استفاده از کانتکست
export const useElementarySchedule = () => {
  const context = useContext(ElementaryScheduleContext);
  if (context === undefined) {
    throw new Error('useElementarySchedule must be used within a ElementaryScheduleProvider');
  }
  return context;
}; 