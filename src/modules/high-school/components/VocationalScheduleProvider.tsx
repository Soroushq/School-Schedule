...
import { VOCATIONAL_GRADES, VOCATIONAL_CLASS_OPTIONS, VOCATIONAL_FIELDS, VOCATIONAL_TEACHING_GROUPS } from '../constants/vocationalConstants';

// تایپ داده‌های کانتکست
interface VocationalScheduleContextType {
  // داده‌ها
...
// ایجاد کانتکست
const VocationalScheduleContext = createContext<VocationalScheduleContextType | undefined>(undefined);

// پراپس‌های کامپوننت Provider
interface VocationalScheduleProviderProps {
  children: React.ReactNode;
}

// کامپوننت Provider
export const VocationalScheduleProvider: React.FC<VocationalScheduleProviderProps> = ({ children }) => {
  // استیت‌های داده
  ...
  // مقدار کانتکست
  const contextValue: VocationalScheduleContextType = {
    // داده‌ها
    personnelList,
    personnelSchedules,
    classSchedules,
    ...
    // ثابت‌ها
    grades: VOCATIONAL_GRADES,
    classOptions: VOCATIONAL_CLASS_OPTIONS,
    fields: VOCATIONAL_FIELDS,
    teachingGroups: VOCATIONAL_TEACHING_GROUPS,
  ...
  return (
    <VocationalScheduleContext.Provider value={contextValue}>
      {children}
    </VocationalScheduleContext.Provider>
  );
};

// هوک استفاده از کانتکست
export const useVocationalSchedule = () => {
  const context = useContext(VocationalScheduleContext);
  if (context === undefined) {
    throw new Error('useVocationalSchedule must be used within a VocationalScheduleProvider');
  }
  return context;
}; 